...
class Action extends Model
{
    ...
    public static function recent(){
        ...
        foreach($actions as $action){
            ...
            switch($action->type){
                case "Create":
                    $recent[] = $action->created_at->toDateTimeString()." (" .$parameters[0].") "."Added Product \"".$parameters[1]."\"";
                break;
                case "Edit":
                    $str = $action->created_at->toDateTimeString()." (" .$parameters[0].") "."Edited Product \"".$parameters[1]."\"";
                    if(isset($parameters[2]) && ($parameters[2] != $parameters[1])){$str .= ", (renamed from \"".$parameters[2]."\")";}
                    $recent[] = $str;
                case "Toggle":
                    $str = $action->created_at->toDateTimeString()." (" .$parameters[0].") "."Toggled Product \".$parameters[1].\""." it is now ";
